# Keycloak Broker: GitHub Social Identity Provider Quickstarts

What is it?
-----------

This example demonstrates how to use Social Identity Providers with Keycloak to authenticate users. In this case,
users are authenticated with GitHub using Keycloak Identity Broker capabilities using the oAuth 2 protocol.
...
In this page you'll note that there is a "GitHub" button that allows you to authenticate with GitHub Identity Provider.

After clicking the "GitHub" button, you'll be redirected to GitHub's login page from where you must authenticate
and grant the necessary permissions to Keycloak in order to access your personal information from GitHub.

If everything is fine, GitHub will redirect you back to Keycloak and at this point you'll be asked to provide some
...
load their profile from GitHub. For that, this application demonstrates how to retrieve the token issued by a social provider
for the authenticated user and use this token to invoke GitHub's API.

Make sure you've set up a application in GitHub
--------------------------------------

This example application requires you to create a GitHub Application. How to create it is beyond the scope of this
documentation.

Please take a look on [GitHub Developer Console](https://github.com/settings/developers) for more details. Make sure to use the correct
Authorization callback URL to be used. GitHub will redirect to this URI after finish authentication. For this example, it's the URL
[http://localhost:8080/auth/realms/github-identity-provider-realm/broker/github/endpoint](http://localhost:8080/auth/realms/github-identity-provider-realm/broker/github/endpoint) .
You can also determine this redirect URI from Keycloak admin console (It's in Identity provider settings for GitHub provider).
Set 'Homepage URL' to 'http://localhost:8080/github-authentication'.

Once you have a GitHub Application configured, you need to obtain both **Client ID** and **Client Secret** and update the
**github-identity-provider-realm.json** configuration file with these information. There you'll find a section as follows:
...
Import the **github-identity-provider-realm.json** file that is in the github-authentication/ example directory.
...
4. This will deploy `target/github-authentication.war` to the running instance of the server.


Access the application
---------------------

The application will be running at the following URL: <http://localhost:8080/github-authentication>.
