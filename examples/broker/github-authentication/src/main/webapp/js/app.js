        ...
        module.factory('authInterceptor', function($q) {
            return {
                request: function (config) {
                    ...
                    if (config.url.startsWith('/') && !config.headers.Authorization) {
                        config.headers.Authorization = 'Bearer ' + keycloakAuth.token;
                    }

                    deferred.resolve(config);

                    if (config.url.startsWith('/') && keycloakAuth.token) {
...
module.controller('GlobalCtrl', function($scope, $http, $location, Auth) {
    ...
    $scope.loadSocialProfile = function() {
        $http.get('/auth/realms/github-identity-provider-realm/broker/github/token').success(function(data) {
